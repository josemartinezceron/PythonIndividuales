...
class CitaManager(): # Equivalente a la Clase Usuario, aca se almacenaran los citas en un archivo JSON
    def __init__(self):
        self.citas = []
        self.cargar_citas()
        self.opciones = [
    ...
    def ver_lista_citas(self):
        if len(self.citas) == 0:
            ...
            for i, cita in enumerate(self.citas):
                print(f"{i + 1}. {cita.paciente} - {cita.fecha} - {cita.hora}")    

    
    def cargar_citas(self):
        citas = []  # Define la variable citas como una lista vacía
        
        try:
            with open("citas.json", "r") as archivo:
                citas_dict = json.load(archivo)
            cita = None  
            ...
            pass  # No es necesario hacer nada en este caso, ya que la variable citas ya está definida como una lista vacía
        
        return citas  # Retorna la variable citas al final del método
    ...
    def guardar_citas(self):
        ...
        with open("citas.json", "w") as archivo:
            json.dump(citas_dict, archivo, indent=4)
